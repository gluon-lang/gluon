...
trait OnFound {
    fn on_ident(&mut self, ident: &ast::TcIdent<Symbol>) { let _ = ident; }
    fn on_expr(&mut self, expr: &ast::LExpr<ast::TcIdent<Symbol>>) { let _ = expr; }
    fn on_pattern(&mut self, pattern: &ast::LPattern<ast::TcIdent<Symbol>>) { let _ = pattern; }
...
impl OnFound for GetType {
    ...
    fn ident(&mut self, _context: &ast::LExpr<ast::TcIdent<Symbol>>, ident: &ast::TcIdent<Symbol>) {
    ...
    fn visit_expr(&mut self, current: &ast::LExpr<ast::TcIdent<Symbol>>) {
        ...
        match current.value {
            ...
            IfElse(ref pred, ref if_true, ref if_false) => {
                self.visit_one([pred, if_true, if_false.as_ref().expect("false branch")].iter().map(|x| &***x))
            }
            ...
            Array(ref array) => {
                self.visit_one(&array.expressions)
            }
            Record { ref exprs, .. } => {
...
pub fn find(env: &DisplayEnv<Ident = ast::TcIdent<Symbol>>, expr: &ast::LExpr<ast::TcIdent<Symbol>>, location: Location) -> Result<TcType, ()> {
...
pub fn suggest(env: &DisplayEnv<Ident = ast::TcIdent<Symbol>>, expr: &ast::LExpr<ast::TcIdent<Symbol>>, location: Location) -> Result<Vec<Symbol>, ()> {
    let mut visitor = FindVisitor {
        env: env,
        location: location,
        on_found: Suggest {

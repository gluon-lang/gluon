...
fn show_forall() {
    ...
    let typ: ArcType<&str> = Type::forall(collect![arg("a"), arg("b")], Type::unit());
...
fn show_record_singleton_polymorphic() {
    ...
    let typ = Type::record(vec![Field::new("Test",
                                           Alias::new("Test",
                                                      Type::forall(collect![Generic::new("a",
                                                                            Kind::typ())],
                                                                   fun.clone())))],
                           vec![]);
...
fn show_record_multiline() {
    ...
    let record = Type::record(vec![Field::new("Test",
                                   Alias::new("Test",
                                              Type::forall(collect![Generic::new("a", Kind::typ())],
                                                           fun.clone())))],
                              vec![Field::new("x", Type::int()),
...
fn show_record_multi_line_nested() {
    ...
    let fun = Type::function(vec![data("a", vec![])], Type::string());
    let test = data("Test", vec![data("a", vec![])]);
    let record_looooooooooooooooooooooooooooooooooong =
        Type::record(vec![Field::new("Test",
                                   Alias::new("Test",
                                              Type::forall(collect![Generic::new("a", Kind::typ())],
                                                           fun.clone())))],
                     vec![Field::new("x", Type::int()),
                           ...
                           vec![Field::new("x", Type::int()),
                                Field::new("test",
                                           Type::function(vec![data("Test",
                                                               ...
                                                               fun.clone()],
                                                          fun.clone())),
                                Field::new("record_looooooooooooooooooooooooooooooooooong",
                                           record_looooooooooooooooooooooooooooooooooong),
                                Field::new("looooooooooooooooooooooooooooooooooong_field",
                                           test.clone())]);
    let expected = r#"{
    Test = forall a . a -> String,
    x : Int,
...
fn show_variant() {
    let typ: ArcType<&str> =
        Type::variant(vec![Field::new("A", Type::function(vec![Type::int()], Type::ident("A"))),
...
fn show_polymorphic_record_associated_type() {
    let type_fields = vec![Field::new("Test",
                                      Alias::new("Test",
                                                 Type::forall(collect![Generic::new("a",
                                                                       Kind::typ())],
                                                              Type::ident("a"))))];
